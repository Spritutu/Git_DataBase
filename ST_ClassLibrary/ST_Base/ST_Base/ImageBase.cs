...
namespace ST_Base
{
    public class ImageBase
    {
        ...
        public void ImagefromCamera(Camera camera)//相機讀取(Image)
        {
            if (Image != null)
            {
                Image.Dispose();
            ...
            HOperatorSet.GenEmptyObj(out Image);
            camera.captrue(out Image);
            HOperatorSet.GetImageSize(Image, out hv_Width, out hv_Height);
        }

        public void ImagefromCameraandshow(Camera camera,HTuple Window)//相機讀取(Image)
        {
            if (Image != null)
            {
                Image.Dispose();
            ...
            camera.captrue(out Image);
            HOperatorSet.GetImageSize(Image, out hv_Width, out hv_Height);
            this.ShowImage_autosize(Window);
        }
        ...
        public void CopyImagetoThis(HObject image)//取得圖片的mean跟deviation
        {
            Image = image.Clone();
        }
    }
